...
ggplot() +
  ...
  geom_text(data = somewhat_rude, aes(x=perc/2, y = type, label = paste0(round(perc*100), "%")),
            size = 4) +
  colorblindr::scale_fill_OkabeIto(name = "")+ #One potential area of improvement: I typically like these colors from OkabeIto, but for this plot, I feel the blue and the green are very similar. Perhaps you could lighten one or the other to make their divergence more clear in the final product? lighten()
  ...
  labs(x = NULL,
       y = NULL,
       title = "Airplane etiquette judgments", #For APA purposes, you might want to capitalize each letter in the title. Just a small thing.
...
figure_1 <- data %>% 
  ...
  mutate(total = no + yes,
         ...
         perc_yes = yes/total) %>% 
  pivot_longer(cols = starts_with("perc"),
               values_to = "perc", #great job here, you really cleaned this up nicely!
               names_prefix = "perc_",
               names_to = "judgment") %>% 
  mutate(n = perc*total,
         perc = ifelse(judgment == "no", perc*-1, perc)) %>% 
...
ggplot() +
  ...
  theme(legend.position = "bottom", 
         legend.direction = "horizontal",
        plot.title.position = "plot")
# I can really tell the difference in cognitive load from directly applying the text to the bars rather than using a legend - great job!!
```
...
figure_2 %>% ggplot(aes(x = perc, y = fct_reorder(type_cat, perc)))+
  ...
  #I am actually having a hard time following this graph because both legends are on the bottom. You might want to consider moving the "reason" legend to the side to more clearly differentiate what the percentages are referring to? But maybe that's just my tired eyes.

#I am also confused as to how you are displaying essential vs. non essential. I understand when reading your code, but you might consider adding some sort of clarification that you have "subgroups" within each category of "bring your child", etc., so that viewers can see the plot and get all the information they need without being walked through it.
```
...
figure_2 %>% 
  ggplot(aes(x = perc, y = fct_reorder(type, perc))) + #This factor reorder makes a big difference in cleaning things up. Nice!
...
#wow, this is amazing how you created your own function so that you could get exactly what you needed for the plot!
...
# So i'm actually wondering if you could flip the axes here and display the means of the judgments with uncertainty, then group by height? I think this could be very informative because right now, the data is not clear what the means and standard errors are for the judgments. But as is, this still is so beautiful and i love the way you clearly annotated with geom_label!
```
...
#I can't get over how creative this is! What a cool idea. 
...
figure_3 %>% 
  ...
  theme_minimal()+
  coord_flip()
 #wow, this plot really came a long way! I can see how box plot was your initial thought for these data, but I think your final product was a much cleaner and more visually appealing way to communicate the finding.
```
...
figure_3 %>% 
  ...
  theme(legend.position = "none",
        plot.title.position = "plot")+
  colorblindr::scale_fill_OkabeIto()
#I agree that the points on this plot version are extremely distracting and overwhelming for readers.
```
...
figure_3 %>% 
  ...
  coord_flip()+
  ggforce::geom_sina(aes(color = rude_recline))+
  labs(y = "Height of judger (in inches)", #This is great practice by including the metric of inches.
       x = NULL,
       title = "How reclining behavior is judged by height",
       subtitle = expression(paste("Is it rude to ", italic("recline"), 
                                   " your seat on an airplane?")))+
